...
//TODO: Goal is to convert existing animation code to store animations as prebuilt matrices, 
//      rather than having to compute_global_transform for every bone at time t

pub struct GltfModel {
    pub meshes : Vec<Mesh>,
    pub skeletons : Vec<Skeleton>,
    pub animations : Vec<Animation>,
    pub current_anim_time : f64,
    pub nodes      : Vec<Node>,
}

impl GltfModel {
	pub fn new( file_path : &str) -> GltfModel {
		...
		for anim in gltf_model.animations() {
            ...
            //TODO: prebake matrices for each keyframe
			//Store the animation
			...
			match node.mesh() {
				Some(gltf_mesh) => {
					...
					for primitive in gltf_mesh.primitives() {
						...
						meshes.push(Mesh::new(vertices_vec, indices_vec, skeleton_index));
					} 
				},
				None => {},
			}
...
impl Mesh {
    ...
    pub fn new(in_vertices : Vec<Vertex>, in_indices : Option<Vec<u32>>, skeleton_index : Option<usize>) -> Mesh {
		Mesh {
